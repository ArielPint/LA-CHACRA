-- Migración 030: revierte el costo mensual del Estado de Resultado de "fecha
-- de OC" (migración 028) a "fecha de factura". La fecha de OC hace que el
-- valor COMPLETO de un contrato (ej. Subc. Terminaciones, $800M) caiga de
-- golpe en el mes en que se emitió esa OC, en vez de reflejar el gasto real
-- mes a mes — eso inflaba marzo (y otros meses) de forma irreal. Factura es
-- el mismo criterio que ya usa el resto de la web (Costo Total en el detalle
-- WIP, Facturado por mes en Por Mes) y refleja el gasto progresivo real.
-- No se persigue igualar los números de ninguna planilla externa: esto es
-- estrictamente cuál dato de nuestra base es el correcto para "costo del mes".
